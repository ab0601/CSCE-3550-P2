...
import {
  ...
  exportJWK,
  SignJWT,
  exportPKCS8,  // keep PKCS#8 export only
} from "jose";
...
/**
 * Generate RSA key, export PKCS#8 (with jose), convert to PKCS#1 (with crypto),
 * store PKCS#1 PEM so the DB literally contains "-----BEGIN RSA PRIVATE KEY-----".
 */
async function generateAndStoreRsaKey({ ttlSeconds }) {
  ...
  // 2) Node crypto -> PKCS#1 PEM
  const keyObj = createPrivateKey(pkcs8Pem);
  const pkcs1Pem = keyObj.export({ type: "pkcs1", format: "pem" }); // "-----BEGIN RSA PRIVATE KEY-----"
...
async function ensureSeedKeys() {
  ...
  await generateAndStoreRsaKey({ ttlSeconds: 60 * 60 });   // +1h active
  await generateAndStoreRsaKey({ ttlSeconds: -5 * 60 });   // -5m expired
}
...
function keyObjectFromDbRow(row) {
  const pem = Buffer.isBuffer(row.key) ? row.key.toString("utf8") : String(row.key); // PKCS#1
  return createPrivateKey(pem); // returns a KeyObject usable by jose
}
...
/* ========= Routes ========= */
// JWKS: only non-expired key(s); keep one active like your original server
app.get(["/jwks", "/.well-known/jwks.json"], async (_req, res) => {
  try {
    const rows = selectAllValidKeys();
    ...
    for (const r of rows) {
      ...
      out.push(jwk);
      break; // expose one active key as before
    }
...
// /auth: sign with active or expired key and label kid compatibly
app.post("/auth", async (req, res) => {
  try {
    const wantExpired =
      "expired" in req.query &&
      (req.query.expired === "" || String(req.query.expired).toLowerCase() === "true");
